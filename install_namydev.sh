 #!/usr/bin/env bash
#
# Try `install_namydev.sh --help` for usage.
#
# (c) 2023 NAMYDEV
#
...
show_usage_and_exit() {
	echo
	echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - HYSTERIA-UDP server install script"
	echo
	echo -e "Usage:"
	echo
	echo -e "$(tbold)Install HYSTERIA-UDP$(treset)"
	...
	echo -e "\t-l, --local <file>\tInstall specified HYSTERIA-UDP binary instead of download it."
	echo -e "\t--version <version>\tInstall specified version instead of the latest."
	echo
	echo -e "$(tbold)Remove HYSTERIA-UDP$(treset)"
	echo -e "\t$0 --remove"
	echo
	...
	if ! is_hysteria_installed; then
		_is_frash_install=1
						...
						if [[ -n "$_is_frash_install" ]]; then
							echo
							echo -e "$(tbold)Congratulation! HYSTERIA-UDP has been successfully installed on your server.$(treset)"
							echo
							...
							echo
								...
								start_services
								echo
								echo -e "$(tbold)HYSTERIA-UDP has been successfully update to $VERSION.$(treset)"
								echo
								fi
}
...
perform_remove() {
	...
	echo
	echo -e "$(tbold)Congratulation! HYSTERIA-UDP has been successfully removed from your server.$(treset)"
	echo
